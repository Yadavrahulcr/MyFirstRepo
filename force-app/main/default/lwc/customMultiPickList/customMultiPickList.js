...
export default class CustomMultiPickList extends LightningElement {
    ...
    onSelect(event) {
       ...
       // this.selectedArray=[]
        if(event.target.value == 'SelectAll') {
            ...
            let v=this.options.find(option => option.label === event.target.value)
            console.log(v)
            this.options.find(option => option.label === event.target.value).checked = event.target.checked;
            //this.selectedArray=selectedArray.push(event.target.value);
        ...
        this.options.forEach((opt)=>{
            if(opt.checked===true){
                this.selectedArray.push(opt.value)
            }
        })
        
       console.log(this.selectedArray);
       this.handleoutput(this.selectedArray,'onselect')
        this.postSelect();
    }
    ...
    onRemove(event) {
        ...
        this.selectedArray.find((item)=>{
            if(item==event.detail.name){
                this.selectedArray.splice(this.selectedArray.indexOf(item),1);
            }
        })
        console.log(this.selectedArray);
        this.handleoutput(this.selectedArray,'onremove')
        this.postSelect();        
    }

    handleoutput(op,sorce){
        console.log(op,sorce);
        if(sorce==='onselect'){
           console.log(op)
           this.dispatchEvent(new CustomEvent('selection',{detail:op}))
        }
