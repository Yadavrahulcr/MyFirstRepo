...
export default class MultipicklistSearch extends LightningElement {
    ...
    //     this.selected.forEach((selectedval,index)=>{
    //         if(element.value===selectedval && this.selectedAll.filter(item =>item.value===selectedval).length===0){
    ...
    //  })
     ...
     this.data.forEach((element,index)=>{
          if(this.selected.includes(element.value)&&!this.selectedAll.some(item=>(item.vlaue===element.value))){
            this.selectedAll.push(element)
          }
     })
     ...
     console.log(JSON.stringify(this.selectedAll));
